...
import com.app.malloc.malloc.AppConst;
...
public class DataManager {
    ...
    public List<AppItem> getApps(Context context, int sort, int offset) {
        ...
        if (manager != null) {
            // 缓存变量
            String prevPackage = "";
            Map<String, Long> startPoints = new HashMap<String, Long>();
            Map<String, ClonedEvent> endPoints = new HashMap<String, ClonedEvent>();
            // 获取事件
            long[] range = AppUtil.getTimeRange(SortEnum.getSortEnum(offset));
            UsageEvents events = manager.queryEvents(range[0], range[1]);
            UsageEvents.Event event = new UsageEvents.Event();
        ...
        if (items.size() > 0) {
            boolean valid = false;
            Map<String, Long> mobileData = new HashMap<String, Long>();
